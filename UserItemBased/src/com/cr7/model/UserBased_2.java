...
public class UserBased_2 implements Model {
	private int USERNUM;
	private int ITEMNUM;
	...
	public UserBased_2(DataSet d,int l){
		NEIGHBOURNUM = l;
		Util.p("");
		initWithDataSet(d);
		computeSimilarity();
	...
	private void computeSimilarity(){					//离线计算
		...
		for(int u=1;u<=USERNUM;u++){
			for(int v=u+1;v<=USERNUM;v++){
				...
				double denomiU=0;
				double denomiV=0;
//				boolean noCommon = true;	//判断两用户是否为没有任何共同评分的情况.但是也有bug:如：u、v共同评分两项均为3，且他们平均分也为3
...
//						noCommon = false;
				...
				if(Math.abs(denomiU)<=0.0001 || Math.abs(denomiV)<=0.0001){
					simMatrix[u][v]=0;
				}else	simMatrix[u][v]=nominator/(Math.sqrt(denomiU*denomiV));
